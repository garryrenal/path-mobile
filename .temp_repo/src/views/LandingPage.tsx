  ...
  const fetchRecentRecords = async () => {
    ...
    try {
      ...
      logAuditAction({
        ...
        collectionName: path,
        details: { hospitalId: selectedHospital?.id, count: snapshot.size }
      });
    } catch (error) {
      try {
        handleFirestoreError(error, OperationType.GET, path);
      } catch (e) {
        console.warn('Suppressed firestore error from crashing:', e);
      }
    } finally {
      setLoading(false);
    }
                ...
                filteredRecords.map(r => (
                  <tr key={r.id} className="hover:bg-slate-50/50 transition-colors cursor-pointer group" onClick={() => onStartTreatment(r.modality, r.patientDetails)}>
                    <td className="px-6 py-4">
                      ...
                      <div className="text-[10px] text-slate-400">{new Date(r.createdAt?.toDate()).toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit' })}</div>
                    </td>
                    <td className="px-6 py-4">
